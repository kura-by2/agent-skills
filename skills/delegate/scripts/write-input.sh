#!/bin/bash
# Usage: write-input.sh <task_name>   (指示本文は stdin から渡す)
# delegate の指示ファイルを /tmp/delegate-inputs/<task_name>.md に作成するだけの薄いラッパ。
...
TASK="${1:-}"
if [ -z "$TASK" ]; then
  echo "Usage: write-input.sh <task_name>  (本文は stdin)" >&2
  exit 1
fi
